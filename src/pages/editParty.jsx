...
export default function EditParties({ data, setData, change, setChange }) {
  ...
  let uid = data.uid;
  useEffect(() => {
    const storedParty = JSON.parse(localStorage.getItem("party"));
    if (storedParty) {
      setPartyName(storedParty.partyName || "");
      setGSTIN(storedParty.GSTIN || "");
      setPhoneNo(storedParty.phoneNo || "");
      setGstType(storedParty.GstType || "");
      setState(storedParty.state || "");
      setEmail(storedParty.Email || "");
      setAdd(storedParty.Add || "");
      setOpeningBalance(storedParty.OpeningBalance || "");
      setAsDate(storedParty.asDate || "");
      setAddF1(storedParty.AddF1 || "");
      setAddF2(storedParty.AddF2 || "");
      setAddF3(storedParty.AddF3 || "");
    }
  }, []);
const editPartiesReq = async () => {
  let updatedParty = {
    partyName: partyName || "",
    GSTIN: GSTIN || "",
    phoneNo: phoneNo || "",
  ...
  // Find the index of the party you want to edit
  const partyIndex = newDa.parties.findIndex((party) => party.GSTIN === GSTIN);

  if (partyIndex !== -1) {
    // If the party exists, update it
    newDa.parties[partyIndex] = updatedParty;
  } else {
    // If the party doesn't exist, add it as a new party
    newDa.parties.push(updatedParty);
  }
  ...
  return (
    <div id="addItem">
      <div className="container">
        ...
        <div className="c3">
          {/* <button onClick={() => editPartiesReq()}>Save & New</button> */}
          <button onClick={() => editPartiesReq()}>Save</button>
        </div>
      </div>
    </div>
  );
